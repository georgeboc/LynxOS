...
typedef struct tagFIS_DMA_SETUP
{
	...
	//uint32_t 1&2
        ...
        //uint32_t 4
        uint32_t   DMAbufOffset;   //uint8_t offset into buffer. First 2 bits must be 0
 
        //uint32_t 5
        uint32_t   TransferCount;  //Number of uint8_ts to transfer. Bit 0 must be 0
...
namespace LY
{
	namespace SATA
	{
		bool Read_Port(HBA_PORT *port, DWORD startl, DWORD starth, DWORD count, DWORD *buf);
		bool Read_ATAPI_Port(HBA_PORT *port, DWORD startl, DWORD starth, DWORD count, DWORD *buf);
		bool Write_Port(HBA_PORT *port, DWORD startl, DWORD starth, DWORD count, DWORD *buf);
		...
		void Init_HBA(HBA_MEM *abar);
		bool Identify_SATA(HBA_PORT *port, DWORD *buf);
		uint32_t Flush_Commands(HBA_PORT *port);
	}
}
